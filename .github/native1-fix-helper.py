...
fn ordinary_coff_runtime_symbol_override_is_rejected() {
    ...
    let llvm_prefix = std::env::var_os("LLVM_SYS_221_PREFIX")
        .map(PathBuf::from)
        .or_else(|| Path::new(env!("CARGO_MANIFEST_DIR")).parent().and_then(Path::parent).map(|root| root.join(".tools/llvm/22.1.8-mingw64")))
        .expect("LLVM prefix");
    ...
    let status = Command::new(llvm_prefix.join("bin/llvm-mc.exe"))
        .arg("-triple=x86_64-w64-windows-gnu").arg("-filetype=obj").arg("-o").arg(&object).arg(&assembly)
        .status().expect("llvm-mc");
    assert!(status.success(), "llvm-mc failed: {status}");
    let ar = std::env::var_os("KELD_AR").unwrap_or_else(|| "ar".into());
    let listing = Command::new(&ar).args(["t", import_library.to_str().expect("import library path")]).output().expect("ar list");
    assert!(listing.status.success(), "ar list failed");
    let first_member = String::from_utf8(listing.stdout).expect("archive member names").lines().next().expect("runtime import archive member").to_owned();
    let object_name = object.file_name().and_then(|name| name.to_str()).expect("override object name");
    let status = Command::new(&ar).current_dir(directory).arg("r").arg(&import_library).arg(object_name).status().expect("ar append");
    assert!(status.success(), "ar append failed: {status}");
    let status = Command::new(&ar).current_dir(directory).arg("mb").arg(&first_member).arg(&import_library).arg(object_name).status().expect("ar move");
    assert!(status.success(), "ar move failed: {status}");
    let output = directory.join("ordinary-coff-override.exe");
    let error = build_executable(&const_module(7), &metadata(), &request(&output, &runtime_dll, &import_library, OptimizationLevel::O0))
        .expect_err("ordinary COFF runtime symbol override must be rejected");
    assert!(matches!(error, BackendError::Toolchain(_)));
    assert!(error.to_string().contains("runtime import library must contain a COFF import"));
...
{"add-test": add_test, "apply-fix": apply_fix}[sys.argv[1]]()
