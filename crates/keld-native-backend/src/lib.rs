...
impl<'module> ScalarLowerer<'module> {
    ...
    fn emit_home_track(&mut self, register: Register) -> Result<(), BackendError> {
        ...
        self.line(format!("br i1 {active}, label %{check}, label %{append}"));
    ...
    fn emit_home_untrack(&mut self, register: Register) -> Result<(), BackendError> {
        ...
        self.line(format!("{active} = icmp ult i32 {index}, {count}"));
        self.line(format!("br i1 {active}, label %{check}, label %{done}"));
        ...
        self.line(format!("{current} = load i32, ptr {pointer}"));
        self.line(format!("{found} = icmp eq i32 {current}, {}", register.0));
        self.line(format!("{source_start} = add i32 {index}, 1"));
        self.line(format!("br i1 {found}, label %{shift}, label %{advance}"));
